...
CEthread_t** hilo_actual_general_flujo = NULL;         // Referencia al hilo actualmente en ejecución en la cola de general (para algoritmo FIFO)
...
CEthread_queue_t* cola_de_listo_general_flujo = NULL;    // referencia a la cola general del calendarizador
short* flag_hilo_actual_actualizado_flujo = NULL;
volatile short* flag_cambio_contexto_RR_flujo = NULL;


void inicializar_parametros_flujo(CEthread_t** hilo_actual_izquierda, CEthread_t** hilo_actual_derecha, CEthread_t** hilo_actual_general,
                                CEthread_queue_t* cola_listo_izquierda, CEthread_queue_t* cola_listo_derecha, CEthread_queue_t* cola_listo_general){
    hilo_actual_izquierda_flujo = hilo_actual_izquierda;
    hilo_actual_derecha_flujo = hilo_actual_derecha;
    hilo_actual_general_flujo = hilo_actual_general;
    cola_de_listo_izquierda_flujo = cola_listo_izquierda;
    cola_de_listo_derecha_flujo = cola_listo_derecha;
    cola_de_listo_general_flujo = cola_listo_derecha;
...
void control_flujo(short param_W, double tiempoLetrero, TipoFlujo algoritmoFlujo, Algoritmos_calendarizacion_en_flujo algoritmo_calendarizacion){
    switch (algoritmoFlujo) {
        ...
        case FLUJO_LETRERO:
            ...
            break;
        case FLUJO_FIFO:
            FIFO(algoritmo_calendarizacion);
            break;
        default:
            equidad(param_W, algoritmo_calendarizacion);
    }
...
void FIFO(Algoritmos_calendarizacion_en_flujo algoritmo_calendarizacion){

    while((*hilo_actual_general_flujo) != NULL){
        (*hilo_actual_general_flujo)->state = RUNNING;  // se actualiza el estado del hilo en ejecutandose
        // se guarda una referencia al hilo que se está ejecutando, para cuando termine, saber su estado en el while sin que se produzca un error, pues hilo_actual cambia
        CEthread_t* hilo_actual_temp = *hilo_actual_general_flujo;

        // Si el algoritmo es RR, entonces cada vez que se coloca a ejeucutar un hilo se reinicia el quantum
        if (algoritmo_calendarizacion == ROUND_ROBIN){
            set_lado_en_cambio_contexto_RR((*hilo_actual_general_flujo)->lado_calle);  // se coloca en qué cola se realiza el cambio de contexto
            set_flag_cambio_contexto_CEthread(0);
            set_reiniciar_timer_RR();
        }
        kill((*hilo_actual_general_flujo)->thread_id, SIGCONT); // pone a ejecutar el hilo
